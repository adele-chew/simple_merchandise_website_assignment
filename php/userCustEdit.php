...
//version 5.0 1/8 3.30AM
	...
	//staff function to edit customer details. checks each variable and updates customer information if post data is retrieved and not empty
	...
	$print .= "<p> Page will reload shortly. </p>";
	echo $print;
	mysqli_free_result($result);


	$query = "SELECT CustName, Email, Contact, Address FROM Customer WHERE CustName = '".$_POST['name']."'";
	$result = $mysqli->query($query);
  	printDetails($result);
  	mysqli_free_result($result);	
	$mysqli -> close();
...
function printDetails($result) {
	if ($result->num_rows > 0) {
		...
		{	
			echo "<p> Customer username: ".$row['CustName']."</p>";
